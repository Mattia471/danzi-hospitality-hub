      ...
      <Container className="grid gap-10 lg:grid-cols-[0.85fr_1.15fr] lg:items-center">
        <div>
          <SectionHeading
            ...
            description="Collaboriamo con brand specializzati per costruire soluzioni complete per il mondo dell’hospitality."
          />
          <Link
            to="/partner"
            className="mt-8 inline-flex items-center gap-3 text-xs font-semibold uppercase tracking-[0.14em] text-brand-orange"
          >
            ...
            <div>
              ...
              <div className="mt-7 flex flex-wrap justify-center gap-2">
                {partners.slice(0, 6).map((partner) => (
                  <span
                    key={partner.id}
                    className="border border-black/10 bg-white px-3 py-2 text-xs text-brand-muted"
                  >
                    {partner.name}
                  </span>
                ))}
